...
export default function IngredientesTable({ pasoId, recetaId, ingredientes, onIngredientesChange }) {
  ...
  useEffect(() => {
    const fetchMateriasPrimas = async () => {
      try {
        ...
        const materiasPrimasData = response.map(mp => ({
  ...
  const handleAddClick = () => {
    if (materiasPrimasDisponibles.length === 0) {
      toast.warning("No hay más materias primas disponibles para agregar");
      return;
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.error(`Error al agregar el ingrediente: ${error.message}`);
    }
  };
