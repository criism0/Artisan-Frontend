          ...
          {data.map(row => (
            <tr key={row.id}>
              ...
              <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-800 flex gap-2">
                {editingRow === row.id ? (
                  <>
                    <button onClick={handleSave} disabled={isSaving} className="text-green-600"><Check /></button>
                    <button onClick={() => setEditingRow(null)} className="text-red-600"><X /></button>
                  </>
                ) : (
                  <>
                    <button onClick={() => handleEdit(row)} className="text-blue-600"><Pencil /></button>
