/**
 * BandejaSII — Bandeja de documentos recibidos desde el SII vía LibreDTE.
 *
 * Muestra las facturas, guías de despacho y otros DTEs que proveedores
 * han emitido a nuestra empresa. Datos provenientes de LibreDTE /dte/dte_recibidos.
 *
 * Filtros:
...
import { Inbox, Search, RefreshCw, FileText, Truck, FileMinus, FilePlus } from 'lucide-react';
...
export default function BandejaSII() {
  ...
  const buscar = useCallback(async () => {
    ...
    try {
      ...
      if (filtros.emisor.trim()) params.emisor = filtros.emisor.trim();
      ...
      toast.error('Error al cargar bandeja SII: ' + (err?.message ?? 'Error desconocido'));
    } finally {
      setLoading(false);
    }
  }, [filtros]);
  ...
  return (
    <div className="p-6 bg-background min-h-screen">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-text">Bandeja SII</h1>
          <p className="text-sm text-gray-500">
            Documentos tributarios recibidos de proveedores vía LibreDTE
          </p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow border border-border p-4 mb-5">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">RUT Emisor</label>
            <input
              type="text"
              name="emisor"
      ...
      <div className="bg-white rounded-xl shadow border border-border overflow-hidden">
        {!buscado && !loading && (
          <div className="flex flex-col items-center justify-center py-16 text-gray-400">
            ...
            <p className="text-sm">Aplica los filtros y presiona <strong>Buscar</strong> para ver documentos recibidos</p>
          </div>
        )}
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="text-left px-4 py-3 font-medium text-gray-600">Emisor</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {documentos.map((doc, i) => (
                  <tr key={`${doc.emisor}-${doc.dte}-${doc.folio}-${i}`} className="hover:bg-gray-50">
