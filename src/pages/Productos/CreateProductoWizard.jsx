...
export default function CreateProductoWizard() {
  ...
  const handleGuardarProducto = async () => {
    ...
    if (!productoForm.descripcion.trim()) {
      toast.error("Descripción es obligatoria");
      return;
    }
    if (!productoForm.codigo_ean.trim()) {
      toast.error("Código EAN es obligatorio");
      return;
    }
    if (!productoForm.codigo_sap.trim()) {
      toast.error("Código SAP es obligatorio");
      return;
    }

    const peso = toNumber(productoForm.peso_unitario);
    const precio = toNumber(productoForm.precio_unitario);
    const upc = Number(productoForm.unidades_por_caja);

    if (peso <= 0) {
      toast.error("Cantidad por unidad debe ser mayor a 0");
      return;
    }
    if (!productoForm.unidad_medida) {
      toast.error("Unidad de medida es obligatoria");
      return;
    }
    if (precio <= 0) {
      toast.error("Precio unitario debe ser mayor a 0");
      return;
    }
    if (!Number.isFinite(upc) || upc <= 0) {
      toast.error("Unidades por caja debe ser mayor a 0");
      return;
    }
    ...
    try {
      ...
      if (productoId) {
        const updated = await api(`/productos-base/${productoId}`, {
          method: "PUT",
          body: JSON.stringify(payload),
        });
        setProductoId(updated?.id ?? productoId);
        toast.success("Producto base actualizado. Ahora crea/actualiza la receta.");
      } else {
        const created = await api("/productos-base", {
          method: "POST",
          body: JSON.stringify(payload),
        });
        setProductoId(created?.id ?? null);
        toast.success("Producto base creado. Ahora crea la receta.");
      }
      ...
      setRecetaForm((prev) => ({
        ...
        peso: prev.peso || String(payload.peso_unitario),
      }));
      setTab("receta");
    } catch (e) {
      console.error(e);
  ...
  const handleGuardarReceta = async () => {
    if (!productoId) {
      toast.error("Primero debes crear el producto base");
      return;
    }
  ...
  return (
    <div className="p-6 bg-background min-h-screen">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap mb-4">
        <div>
          <h1 className="text-2xl font-bold text-text">Crear Producto Base</h1>
      ...
      {tab === "datos" ? (
        <div className="bg-white p-6 rounded-lg shadow space-y-4">
          <div className="text-sm font-semibold text-gray-800">Datos del producto base</div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setProductoForm((p) => ({ ...p, precio_unitario: e.target.value }))}
              />
            </div>
          </div>
      ...
      {tab === "receta" ? (
        <div className="bg-white p-6 rounded-lg shadow space-y-4">
          <div className="text-sm font-semibold text-gray-800">Receta del producto base</div>
          <div>
