...
export default function LotesList() {
  ...
  const columns = [
    {
      ...
      Cell: ({ row }) => (
        ...
        >
          {expandedRows.has(row.id) ? <ChevronDown /> : <ChevronRight />}
        </button>
      ),
    },
    {
  ...
  useEffect(() => {
    if (!canReadInProgress && !canReadFinished) {
      toast.permissionError([ModelType.LOTE_PRODUCTO_EN_PROCESO, ScopeType.READ], [ModelType.LOTE_PRODUCTO_FINAL, ScopeType.READ]);
      setIsLoading(false);
      setLotes([]);
      setFiltered([]);
      return;
  ...
  if (isLoading) return <PageLoader message="Cargando lotes" />;
