...
export default function ValidarOrden() {
  ...
  useEffect(() => {
    ...
    const fetchOrden = async () => {
      try {
        const response = await api(`/proceso-compra/ordenes/${ordenId}`);
        
        const insumos = response.materiasPrimas.map(mp => ({
          id: mp.id,
        ...
        setOrdenData({
          ...response,
          proveedor: response.Proveedor?.nombre_empresa || response.id_proveedor,
          lugar: response.BodegaDestino?.nombre || "—",
          numero: `OC-${String(response.id).padStart(3, "0")}`,
          fecha_emision: response.fecha,
          insumos,
        });
      } catch (error) {
  ...
  const handleValidar = async () => {
    ...
    try {
      await api(`/proceso-compra/ordenes/${ordenId}/validar`, { method: "PUT" });
