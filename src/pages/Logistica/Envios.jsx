  ...
  const fetchSolicitudes = async () => {
    ...
    try {
      const urls = [
        ...
        `/solicitudes-mercaderia/validada`,
        `/solicitudes-mercaderia/en-transito`,
      ];

      const responses = await Promise.all(urls.map((url) => api(url)));
      const all = responses.flatMap((r) => (Array.isArray(r) ? r : []));
      setSolicitudes(all);
    } catch (err) {
